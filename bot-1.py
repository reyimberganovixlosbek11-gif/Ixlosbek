...
def flow_receive_episode_video(message):
    # Video sifatida ham, dokument (fayl) sifatida ham qabul qilamiz
    file_id = None
    ...
    if not file_id:
        msg = bot.send_message(
            ...
            "<i>Eslatma: Telegram'da yuborishdan oldin \"Fayl sifatida yuborish\" tugmasini bosmang, "
            "oddiy video sifatida yuboring.</i>",
            parse_mode="HTML",
        )
        bot.register_next_step_handler(msg, flow_receive_episode_video)
        return
    ...
    logger.info("Admin added episode: anime=%s ep=%s", anime_code, ep_num)

    eps = db.get_episode_numbers(anime_code)
    bot.send_message(
        message.chat.id,
        f"✅ <code>{anime_code}</code> — <b>{ep_num}-qism</b> bazaga qo'shildi!\n\n"
        f"📋 Jami qismlar: {eps}",
        reply_markup=admin_panel_markup(),
        parse_mode="HTML",
    )
...
def handle_photo(message):
    # Only used inside register_next_step_handler flows; ignore standalone
    pass


@bot.message_handler(content_types=["video", "document"])
def handle_video_standalone(message):
    # Admin uchun file_id ni ko'rsatish (debug)
    if not is_admin(message.from_user.id):
        return
    if message.content_type == "video" and message.video:
        bot.reply_to(
