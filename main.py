...
async def m_del_code(message: Message, state: FSMContext):
    ...
    if not movie:
        await state.clear()
        return await message.answer("❌ Topilmadi.", reply_markup=admin_menu())
...
async def send_movie_by_code(message: Message, bot: Bot, code: int):
    ...
    kb_list = []
    # Kino qo'shilganda yuklangan asosiy video har doim ko'rsatiladi
    kb_list.append([InlineKeyboardButton(text="🎬 Kinoni ko'rish", callback_data=f"watch_{code}")])
    if episodes:
        # Qismlar tugmalari (qo'shimcha)
        row = []
