...
async def start_broadcast(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id): return
    await call.message.edit_text("📝 Reklama xabarini yuboring (Matn, rasm, video bo'lishi mumkin):")
...
async def remove_sub_channel(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id): return
    await call.message.edit_text("📢 O'chirish uchun kanal username ini kiriting (masalan: @MyChannel):")
...
async def cb_add_movie(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id): return
    await call.message.edit_text("🔢 Kino uchun noyob kod (raqam) kiriting:")
...
async def m_imdb(message: Message, state: FSMContext):
    ...
    await message.answer("🌍 Ishlab chiqarilgan davlatni kiriting:")
...
async def m_country(message: Message, state: FSMContext):
    ...
    await message.answer("🗣 Tilini kiriting (masalan: O'zbekcha):")
...
async def m_genre(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("📝 Kino haqida qisqacha tavsif (tavsif matni) kiriting:")
...
async def m_desc(message: Message, state: FSMContext):
    ...
    await message.answer("🖼 Kino posterini (Rasm) yuboring:")
...
async def m_poster(message: Message, state: FSMContext):
    ...
    await message.answer("📹 Kino videosini yoki Treylerini yuboring:")
    await state.set_state(AddMovie.trailer)

@admin_router.message(AddMovie.trailer, F.video, F.from_user.id.in_(ADMIN_IDS))
async def m_trailer(message: Message, state: FSMContext):
    data = await state.get_data()
...
async def cb_add_ep(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id): return
    await call.message.edit_text("🔢 Qism qo'shiladigan kino kodini kiriting:")
...
async def m_ep_code(message: Message, state: FSMContext):
    ...
    if not get_movie_by_code(code): return await message.answer("❌ Bunday kodli kino topilmadi. Qaytadan kiriting:")
...
async def m_ep_num(message: Message, state: FSMContext):
    ...
    await message.answer("📹 Ushbu qism videosini yuboring:")
...
async def cb_edit_movie(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id): return
    await call.message.edit_text("📝 Tahrirlanadigan kino kodini kiriting:")
...
async def m_edit_code(message: Message, state: FSMContext):
    if not message.text.isdigit(): return await message.answer("Faqat raqam!")
    code = int(message.text)
    if not get_movie_by_code(code): return await message.answer("Topilmadi. Qaytadan kiriting:")
...
async def cb_edit_field(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(f"📝 Yangi qiymatni yuboring:")
