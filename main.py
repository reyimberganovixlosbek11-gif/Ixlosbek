...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8841089797:AAEFYQemX45UoniTLPzdOpiMNmX7JbcS__M")
ADMIN_IDS = [6222096713]
DB_PATH = "kino.db"
CHANNEL_USERNAME = "@Jangari_Kinolaarr"
...
async def movie_file_input(message: Message, state: FSMContext, bot: Bot):
    ...
    logging.info(f"Kino saqlanmoqda: {data}")
    await add_movie(
    ...
    if data.get("poster_file_id"):
        try:
            caption = (
                f"🎬 Nomi: {data['name']}\n\n"
                f"📅 Yili: {data['year']}\n"
                f"📺 Sifati: {data['quality']}\n"
                f"⭐ IMDb: {data['imdb']}\n"
                f"🌍 Davlati: {data['country']}\n"
                f"🇺🇿 Tili: {data['language']}\n"
                f"🎭 Janri: {data['genre']}\n\n"
                f"🔢 FILM KODI: {data['code']}\n\n"
                f"‼️ DIQQAT! Filmni tomosha qilish uchun @JangariKinolaar_bot ga «{data['code']}» kodini yuboring!"
            )
            await bot.send_photo(CHANNEL_USERNAME, data['poster_file_id'], caption=caption)
        except Exception as e:
            await message.answer(f"⚠️ Kanalga post yuborishda xato: {e}")
...
async def main():
    await init_db()
    # Begona kanallarni tozalash
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("DELETE FROM channels WHERE channel_id NOT LIKE '%Jangari%'")
        await db.commit()
